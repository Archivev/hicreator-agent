---
name: hicreator
description: Find Instagram, YouTube, or TikTok creators, find similar creators or public contact emails, and organize creators in hiCreator folders. Use when the user asks for creator discovery, creator email lookup, or saving creator results to hiCreator.
---
...
4. To save results, call `list_creator_folders`; create the destination with `create_creator_folder` only when needed; then call `add_creators_to_folder` once with stable `platform` and `creator_id` values from search results.
...
6. Report returned count, verified constraints, folder result, and any per-item failures.
...
Folder listing, creation, and batch add are free. A failed free folder operation may be retried without repeating the paid search. Do not claim support for folder deletion, rename, removal, or sharing.
